...
def restart_song(params, context):
    """Service for resuming a pending song or rehearsal for a user.

    Invoked when the game was restarted during a song or rehearsal and
    the user chooses to continue.
    Args:
        params: A dict containing the following keys.
            live_token: A string representing the previous live token
                        generated by the game when starting the song or
                        rehearsal.
            second: Number of seconds since the beginning of the song or
                    rehearsal.
            is_two_d: Whether the user chooses to continue the song or
                      rehearsal in 2D mode.
    Returns:
        A dict containing a single key named 'pending_song', whose value
        is a dict representing pending song or rehearsal info. See the
        return value 'pending_song' of the method
        'UserService.GetPendingData' for the dict definition.
    """
    with Session(engine) as session:
        user = session.scalars(
            select(User)
...
def finish_song(params, context):
    ...
    return {
        ...
        'combo_rank': 4 if is_live_support else combo_rank,
        'count_list': params['count_list'],
        'is_full_combo': False if is_live_support else params['is_full_combo'],
...
@dispatcher.add_method(name='LiveService.StartRehearsal',
                       context_arg='context')
def start_rehearsal(params, context):
    """Service for starting a rehearsal for a user.

    Invoked when the user presses Start Rehearsal button.
    Args:
        params: A dict containing the following keys.
            unit_num: Unit number of the chosen unit (1-18).
            use_song_unit: Whether the song unit should be used.
    ...
    with Session(engine) as session:
        ...
        rehearsal_cost = session.scalar(
            select(MstGameSetting.rehearsal_cost)
        )
        user.money -= rehearsal_cost
        if user.money < 0:
            raise RuntimeError('money cannot be negative')
...
def break_rehearsal(params, context):
    ...
    Invoked when the game was paused or restarted during a rehearsal,
    and the user chooses to give up.
    Args:
        params: A dict containing the following keys.
            score: 0.
            score_rank: 0.
            ...
            combo_rank: 0.
            count_list: [].
            life: Current vitality of the user. If the game was
                  restarted during the rehearsal, this value is 0.
            seconds: Number of seconds since the beginning of the
                     rehearsal. If the game was restarted during the
                     rehearsal, this value is 0.
    Returns:
        See the implementation below.
    """
    with Session(engine) as session:
...
def finish_rehearsal(params, context):
    ...
    Invoked when a rehearsal ends.
    Args:
        params: A dict containing the following keys.
            score: 0.
            score_rank: If the game was restarted during the rehearsal,
                        this value is 5. Otherwise, this value is 0.
            combo: Highest combo obtained for this rehearsal.
            max_combo: Maximum possible combo (same as total number of
                       notes).
            count_list: A list of 6 ints representing the number of
                        PERFECT, GREAT, GOOD, SLOW, FAST and MISS notes.
            ...
            sp_appeal_success: Whether the special appeal note was
                               successfully pressed.
            live_token: A string representing the previous live token
                        generated by the game when starting the
                        rehearsal.
    Returns:
        A dict containing the following keys.
        song: A dict representing the song info. See the return value
              'song_list' of the method 'SongService.GetSongList' for
              the dict definition.
        is_event_tour: false.
        is_event_twin_stage: false.
        played_event_type: 0.
