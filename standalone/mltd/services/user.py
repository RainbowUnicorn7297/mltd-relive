...
def get_pending_data(params, context):
    ...
    Returns:
        ...
        pending_song: A dict representing the pending song (empty info
            ...
            is_valid: true only if the pending song was started on the
                      same device.
            retry_count: The total number of retries for this song.
        pending_job: A dict representing the pending job (empty info if
                     no pending job). Contains the following keys.
            ...
            is_challenge_good: Whether the result of the challenge event
                               is good.
            is_valid: true only if the pending job was started on the
                      same device.
    """
    result = {
        'pending_song': {
    ...
    with Session(engine) as session:
        ...
        if user.pending_song:
            ...
            if params['live_token'] != user.pending_song.live_token:
                # Game and server live_tokens do not match if different
                # devices were used. Pending song info should still be
                # returned regardless.
                result['pending_song']['is_valid'] = False
        if user.pending_job:
            ...
            if params['job_token'] != user.pending_job.job_token:
                # Game and server job_tokens do not match if different
                # devices were used. Pending job info should still be
                # returned regardless.
                result['pending_job']['is_valid'] = False
